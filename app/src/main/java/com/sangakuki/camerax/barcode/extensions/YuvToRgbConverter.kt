...
class YuvToRgbConverter(context: Context) {
    ...
    // I420或YV12顺时针旋转90度
    fun rotateClockwise90(src: ByteArray, w: Int, h: Int): ByteArray {
        val dest = ByteArray(w * h)
        var k = 0
        for (i in 0 until w) {
            for (j in h - 1 downTo 0) {
                dest[k++] = src[j * w + i] //旋转Y
            }
        }
        return dest
    }
    ...
    // I420或YV12顺时针旋转270度
    fun rotateClockwise270(src: ByteArray, w: Int, h: Int): ByteArray {
        val dest = ByteArray(w * h)
        var k = 0
    ...
    // I420或YV12顺时针旋转180度
    fun rotateClockwise180(src: ByteArray, w: Int, h: Int): ByteArray {
        val dest = ByteArray(w * h)
        var pos = 0
        var k = w * h - 1
        while (k >= 0) {
